...
struct tp_handle {
	...
	int (*th_client_main)(const char *, const char *, const char *, int, char * const []);
...
tp_handle_register(const char *protostr,
    int (*client)(const char *, const char *, const char *, int, char * const []),
    ...
    const char *filename, int argc, char * const argv[])
{
	...
	return (*th->th_client_main)(addrstr, servstr, filename, argc, argv);
}
